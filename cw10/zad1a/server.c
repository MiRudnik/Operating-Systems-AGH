...
{
    if (argc != 3){
        printf("Pass port number and local name!\n");
        exit(EXIT_FAILURE);
    }
    atexit(cleanup);
    ...
    if (bind(inet, (struct sockaddr *)&(addr), sizeof(addr)) != 0) {
        printf("Couldn't bind to INET.\n");
        exit(EXIT_FAILURE);
    }

    int listen1 = listen(inet, 8);
    if (listen1 !=0){
        printf("Couldn't listen to INET.\n");
        exit(EXIT_FAILURE);
    }
    ...
    {
        printf("Couldn't start epoll for INET.\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (bind(local, (struct sockaddr *)&(address), sizeof(address)) != 0){
        printf("Couldn't bind in LOCAL.\n");
        exit(EXIT_FAILURE);
    }

    listen1 = listen(local, 7);
    if (listen1 != 0){
        printf("Couldn't LISTEN in LOCAL.\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (epoll_ctl(epolld, EPOLL_CTL_ADD, local, &e) <0){
        printf("Couldn't start epoll for LOCAL.\n");
        exit(EXIT_FAILURE);
    }
