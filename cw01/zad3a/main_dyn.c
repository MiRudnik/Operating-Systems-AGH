...
void fillArray_S(struct arrayS* array){
    void (*f_addBlock_S)(struct arrayS*,char*,int);
    f_addBlock_S = (void (*)(struct arrayS*,char*,int))dlsym(handle, "addBlock_S");
    for(int i=0;i<array->size;i++){
        char* block = generateBlock(array->length);
        f_addBlock_S(array,block,i);
    }
}

void fillArray_D(struct arrayD* array){
    void (*f_addBlock_D)(struct arrayD*,char*,int);
    f_addBlock_D = (void (*)(struct arrayD*,char*,int))dlsym(handle, "addBlock_D");
    for(int i=0;i<array->size;i++){
        char* block = generateBlock(array->length);
        f_addBlock_D(array,block,i);
    }
}
...
void add_S(struct arrayS* array, int number){
    ...
    for(int i=0;i<number;i++){
        ...
        char* block = generateBlock(array->length);
        f_addBlock_S(array,block,index);
    }
}
...
void add_D(struct arrayD* array, int number){
    ...
    for(int i=0;i<number;i++){
        int index = rand()%(array->size);
        char* block = generateBlock(array->length);
        f_addBlock_D(array,block,index);
    }
}

void remove_S(struct arrayS* array, int number){
    void (*f_deleteBlock_S)(struct arrayS*,int);
    f_deleteBlock_S = (void (*)(struct arrayS*,int))dlsym(handle, "deleteBlock_S");
    for(int i=0;i<number;i++){
        int index = rand()%(array->size);
        f_deleteBlock_S(array,index);
    }
}

void remove_D(struct arrayD* array, int number){
    void (*f_deleteBlock_D)(struct arrayD*,int);
    f_deleteBlock_D = (void (*)(struct arrayD*,int))dlsym(handle, "deleteBlock_D");
    for(int i=0;i<number;i++){
        int index = rand()%(array->size);
        f_deleteBlock_D(array,index);
    }
}
...
int main(int argc, char **argv){
    ...
    handle = dlopen("./libBlockLibrary.so", RTLD_LAZY);
    if (!handle){
    ...
    if(isStatic){
        ...
        struct arrayS* (*f_createBlockArray_S)(int,int);
        f_createBlockArray_S = (struct arrayS* (*)(int,int))dlsym(handle, "createBlockArray_S");
        struct arrayS* myArray = f_createBlockArray_S(size,length);
        fillArray_S(myArray);
        ...
        while(argCounter < argc){
            if(strcmp(argv[argCounter],"search") == 0){
                ...
                if(number == 0){
                    ...
                    int (*f_searchClosestAscii_S)(struct arrayS*,int);
                    f_searchClosestAscii_S = (int (*)(struct arrayS*,int))dlsym(handle, "searchClosestAscii_S");
                    int result = f_searchClosestAscii_S(myArray,number);
        ...
        struct arrayD* (*f_createBlockArray_D)(int,int);
        f_createBlockArray_D = (struct arrayD* (*)(int,int))dlsym(handle, "createBlockArray_D");
        struct arrayD* myArray = f_createBlockArray_D(size,length);
        fillArray_D(myArray);
        ...
        while(argCounter < argc){
            if(strcmp(argv[argCounter],"search") == 0){
                ...
                if(number == 0){
                    ...
                    int (*f_searchClosestAscii_D)(struct arrayD*,int);
                    f_searchClosestAscii_D = (int (*)(struct arrayD*,int))dlsym(handle, "searchClosestAscii_D");
                    int result = f_searchClosestAscii_D(myArray,number);
                    if(result == -1) printf("Wrong index or there is only one block in the array.\n\n");
                    else printf("Block at index %d is closest in value to block at index %d.\n\n",result,number);
